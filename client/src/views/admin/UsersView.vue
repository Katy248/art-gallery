...
<template>
    <div class="flex gap-4 mb-4">
        <button class="btn btn-base" title="Обновить список" @click="getData(route.params.page)">
            <i class="fas fa-rotate-right"></i>
        </button>
    </div>
    <div class="flex flex-col">
        <button v-for="user in users" :key="user.id" @click="showModal(user)"
    ...
    <TransitionRoot as="template" :show="_showModal" enter="ease-out duration-200" enter-from="opacity-20"
        ...
        <Dialog @close="_showModal = false" class="relative z-50">
            <div class="fixed inset-0 flex w-screen items-center justify-center backdrop-blur-sm p-8">
                <DialogPanel>
                    <div class="bg-bg-2 p-8 rounded-lg flex flex-col gap-6 border-ui border grow">
                        <div>
                            <h2 class="text-lg">
                                {{ currentUser.name }}
                            </h2>
                        ...
                        <div class="">
                            Дата регистрации: {{ new Date(currentUser.createdAt).toLocaleDateString() }}
                        </div>
